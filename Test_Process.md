...
## 主界面
* 若是第一次启动，此时会显示导入或扫描，公告或更新，关闭后或下一次启动就不会显示
* 依次点击侧边栏
...
* 对着 第一个影片 右键 - 添加到已有的标签-选择标签 test1 ，然后进入影片详细信息，查看标签是否更新
* 同理对剩下的标签管理依次测试
...
* 右键取消选择所有
* 右键仅可播放，总数目减少了
* 右键-仅显示分段视频，总数目减少了 取消勾选 仅可播放、仅分段视频 
* 右键-图片模式-有图、无图、回到默认
...
* 选择所有视频 搜索 ZUKO 回车 勾选再检索 搜索 87 取消再检索
* 选择所有视频 勾选名称  搜索 女 
* 选择所有视频 勾选演员  搜索 木
...
* 点击清单 1 对着第一个影片右键，只能显示移出清单 移除第一个
...
* 打开设置-扫描-文件大小-改为 0 MB 并勾选同时导入扫描的 NFO 保存
* 影视库- 新建库 scan1 -主界面选择 scan1
* 工具- 视频扫描-将【常规扫描】文件夹拖入主界面-执行-查看导入结果是否为 19
* 全盘扫描- 执行
* 工具- 选择 access 数据导入，将 Maindatabase.mdb 拖入 成功后，点一下所有视频，正常应增加了 801 个
* 工具-选择 NFO 导入- 勾选复制 poster 与 fanart，拖入【测试 NFO】文件夹，成功后 显示导入个数 2
* 选择导入单个 NFO ：将 IPX-050 拖入，执行
* 欧美影片扫描：将【欧美扫描】文件夹拖入，成功导入 2，且视频类型显示为欧美
* 网络磁盘扫描（待测）
* 库管理，重置当前库 
* 主界面，将常规扫描和测试 NFO 文件夹一起拖入，显示 nfo 2 个，视频 19 个

## 影视库测试
* 设置-扫描：将【常规扫描】拖入到启动时扫描目录中，保存
* 库管理-勾选 删除不位于启动目录中的影片 执行 发现删除了 2 个
...
* 设置-显示-取消勾选名称为空显示文件名-然后 按名称排序保证有影片是有标题的-然后-对着单个影片右键-翻译标题和摘要（设置里勾选上显示翻译后的结果）
